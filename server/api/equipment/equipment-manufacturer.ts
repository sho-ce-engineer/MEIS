...
export default defineEventHandler(async (event) => {
  ...
  if (AuthenticatedUserFacilityResult.rowCount === 0) {
    console.error(
      `[equipment-manufacturer] Facility code not found for user_id: ${authenticatedUser.user_id}`,
    );
    throw createError({
      statusCode: 404,
  ...
  try {
    const result = await pool.query(
      'SELECT DISTINCT equipment_manufacturer FROM equipment_ledger WHERE facility_code = $1',
      [facilityCode],
    );
